...
EOF
  ...
  HAUL_ARCHIVE="hauler_airgap_$(date '+%m_%d_%y').zst"
  touch ${TMPDIR}/${HAUL_ARCHIVE} > /dev/null 2>&1 || { fatal "Unable to create ${TMPDIR}/${HAUL_ARCHIVE} file - check permissions" ; }
  warn "- compressing all the things - will take a minute"
  set +e   # Turn off exit-on-error so we can catch errors tar throws.
  tar -I zstd -cf ${TMPDIR}/${HAUL_ARCHIVE} $(ls) > /dev/null 2>&1
  EC=$?
  set -e   # Re-enable exit-on-error
  [ $EC -gt 0 ] && fatal "Unable to populate ${TMPDIR}/${HAUL_ARCHIVE} archive - check disk space."
  echo -n "  - created ${TMPDIR}/${HAUL_ARCHIVE} "; info_ok
  ...
  echo -e "      tar -I zstd -vxf ${HAUL_ARCHIVE} -C ${TMPDIR}/hauler"
  echo -e "      $0 control"
  echo -e "---------------------------------------------------------------------------"
